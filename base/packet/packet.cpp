...
void Packet::Init(uint32_t opcode, uint32_t size)
{
	...
	memcpy(m_data, (char*)&m_opcode, PACKET_OPCODE_SIZE);
}
...
void Packet::PackageData()
{
	...
	memcpy(m_data + PACKET_OPCODE_SIZE, (char*)&m_data_length, PACKET_DATA_LENGTH_SIZE);
}
...
bool Packet::UnpackageData(const char *buf, uint32_t size)
{
	...
	memcpy(opcode_buf, m_data, PACKET_OPCODE_SIZE);
	m_opcode = *((uint32_t*)opcode_buf);

	char data_length_buf[PACKET_DATA_LENGTH_SIZE] = {0};
	memcpy(data_length_buf, m_data + PACKET_OPCODE_SIZE, PACKET_DATA_LENGTH_SIZE);
